...
# Create DAG  
 ...
 DAG_DO_Ice <- dagify(
      ...
      Rate_O2_Drawdown ~ ER_under_ice + GPP_under_ice + Lake + Year,
        ...
        Light_Availability ~ Snow_Accumulation + Lake + Year,
        Snow_Accumulation ~ Lake + Year,
 ...
 DAG_DO_Ice_change <- dagify(
      
      #Identify Relatiobships 
      Change_in_DO ~ Starting_DO + Days_Since_Freeze + Rate_O2_Drawdown,
      Starting_DO ~ Lake + Year + Days_Since_Freeze,
      Rate_O2_Drawdown ~ Respiration + Photosynthesis + Lake + Year,
        Photosynthesis ~ Light_Availability,
        Light_Availability ~ Recent_Snow + Lake + Year,
        Recent_Snow ~ Lake + Year,
      
      # Specify exposure and outcome 
      exposure = "Recent_Snow", 
      outcome = "Change_in_DO",
      
      # Label 
      labels = c(outcome = "Change_in_DO",
                 exposure = "Recent_Snow"))
 
  DAG_DO_Ice_updated <- dagify(
      
      #Identify Relatiobships 
      Change_in_DO ~ Start_DO + Rate_Photosynth + Rate_Respiration,
      Rate_Photosynth ~ Light_Availability + Phyto_Community + Lake + Year, 
      Light_Availability ~ Recent_Snow + Wind + DOY + Lake + Clouds, 
        Wind ~ Lake + Year,
        Recent_Snow ~ DOY + Year,
...
dag_plot <- ggdag_status( DAG_DO_Ice_change,
             use_labels = "label",
             text = TRUE,
              text_col = "black",
...
adjustmentSets(DAG_DO_Ice_updated)

adjustmentSets(DAG_DO_Ice_change)
