...
simulate_data <- function(){
  ...
  df <- data.frame(id = seq(1, 1000), # ID is just a sequence from 1 to 1000
                   ...
                   slope = runif(1000, min = 50, max = 100), # Here we are generating random numbers for slope between 50 and 100 
  ...
  ### Add column for the treatment variable, related to the IV
    mutate(forest_protect = 0.2*dist_tributary + 0.4*local_inst + # so we know that the treatment variable really is related to the IV in this way 
             rnorm(100, 0, 1)) %>%
  
  ### Add column for the outcome variable
    mutate(consumpt = 5*forest_protect + 
             0.07*slope + 0.05*elevation + 2*dist_cities + 
             10*local_inst + error) # Then this 10 is the number we want to pay attention to, that is the actual relationship between the unobserved variable                                         and the outcome 
  return(df)  
}
```
