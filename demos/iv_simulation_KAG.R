...
z <- rnorm(100, 3, 7) # Exogenous like exo-planet! Coming from outside the system
o <- rnorm(100, 3, 5)
u <- rnorm(100, 0, 7)  # assuming random error and uncorrelated by generating this structure

# now generate endogenous variables (x and y)
# assume it depends linearly on o and z with some noise and with some coefficients
    ...
    beta_ox = 5  #size of bias from omitted variable o depends on this coefficient as well as beta_oy (they compound)
        # This is why having a "bad" instrumental variable can actually be worse! you are just adding more bias
...
coeftest(OLS)  #biased up, because positive effect of o on y and on x.
    # This comes out as 4.9, but we set it to 1! That is because there is bias in this ommitted variable

#Now let's do a model where we control for o., assuming you observe o.
OLS2 <- lm(y ~ x + o)
coeftest(OLS2)
    # Now we do way better! Now we are estimating 1.01
    # But ... that is assuming that we actually measured the ommitted variable ... not always the case

# Third model: IV; we don't observe o but we have data on z.
iv <- ivreg(y ~ x | z)
summary(iv)  # Enter the hero instrumental variable! We still over estimate (comes out to about 2) but way better thansummarysummary(iv)              # normal OLS and does NOT require us to know anything about o
...
linearHypothesis(iv, c(0,1), rhs=1) #? Help?
...
linearHypothesis(OLS, c(0,1), rhs=1)  # OLS vlaue is significantly different from 1
...
#
# We can re-write the system of equations like this using simple substitution: Algebra
